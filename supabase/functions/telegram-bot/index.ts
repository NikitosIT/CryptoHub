...
if (!SUPABASE_URL || !SERVICE_KEY || !TG_TOKEN) {
  safeLogError("Missing env vars", "telegram-bot");
...
async function safeParse(req: Request) {
  try {
    ...
    safeLogError(e, "telegram-bot: parse");
    return null;
  }
}
...
Deno.serve(async (req) => {
  ...
  const srcChat = msg.forward_from_chat ?? msg.sender_chat ?? msg.chat;
  ...
  const { data, error } = await sb.rpc("upsert_telegram_posts", {
    p_text_caption: text,
  ...
  if (error) {
    safeLogError(error, "telegram-bot: upsert");

...
function ListTokens() {
  ...
  return (
    <div className="max-w-2xl px-4 py-6 mx-auto">
      <h1 className="mb-6 text-2xl font-semibold text-white">Cryptotokens List</h1>
      ...
      <ul className="divide-y divide-gray-700">
        {data?.map((token) => {
          ...
          return (
            <li key={token.id} className="flex items-center gap-4 py-4 first:pt-0">
              <img
                src={token.image}
                alt={token.name}
                className="object-contain w-10 h-10 bg-black rounded-full shrink-0"
              />
              <span className="flex-1 font-medium text-white">{token.name}</span>
              <button
                onClick={() => {
                  handleAddOne(token.name);
                }}
              ...
              >
                <img className="w-6" src="/free-icon-send-3106856.png" alt="send" />
              </button>

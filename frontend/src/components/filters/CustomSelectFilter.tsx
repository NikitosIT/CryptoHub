...
function AuthorImg({ id, label }: { id: number; label: string }) {
  return (
    <img
...
function TokenImg({ label }: { label: string }) {
  return (
    <img
...
export default function SelectFilter<T extends OptionType>({
  ...
  const handleChange = (_: unknown, val: T | null) => onChange(val ?? null);
    ...
    return (
      ...
      >
        <Stack direction="row" spacing={1.5} alignItems="center">
          {showLogos === true && <TokenImg label={option.label} />}
        ...
        startAdornment: (
          <>
            ...
            {value !== null &&
              ...
              value.id !== undefined && (
                <InputAdornment position="start">
                  <AuthorImg id={value.id} label={value.label} />
                </InputAdornment>
              )}
